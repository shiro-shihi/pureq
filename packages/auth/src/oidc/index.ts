...
function isRefreshableOidcAccount(account: AuthAccount): boolean {
  return (
...
/**
 * Refresh provider account tokens when expiry is near.
 * Useful for Google/OIDC access token rollover before API calls.
 */
export async function refreshOIDCAccountIfNeeded(
  account: AuthAccount,
  options: OIDCAccountAutoRefreshOptions
): Promise<OIDCAccountAutoRefreshResult> {
  const nowMs = options.now ? options.now() : Date.now();
  const thresholdSeconds = options.refreshThresholdSeconds ?? 60;
  const thresholdMs = Math.max(0, thresholdSeconds) * 1000;

  if (!isRefreshableOidcAccount(account)) {
  ...
  try {
    const tokenResponse = await options.flow.refresh(account.refreshToken!);
    const refreshedAccount: AuthAccount = {
      ...account,
      accessToken: tokenResponse.accessToken,
      refreshToken: tokenResponse.refreshToken ?? account.refreshToken ?? null,
      ...(tokenResponse.expiresIn !== undefined ? { expiresAt: Math.floor(nowMs / 1000) + tokenResponse.expiresIn } : {}),
      ...(tokenResponse.tokenType !== undefined ? { tokenType: tokenResponse.tokenType } : {}),
      ...(tokenResponse.scope !== undefined ? { scope: tokenResponse.scope } : {}),
      ...(tokenResponse.idToken !== undefined ? { idToken: tokenResponse.idToken } : {}),
    };

    await options.onRefreshed?.({ previous: account, next: refreshedAccount, tokenResponse });
    ...
    await options.onError?.(normalized, account);
    throw normalized;
  }
}

/**
 * Refresh and persist a stored OIDC/OAuth account in an adapter when expiry is near.
 */
export async function refreshStoredOIDCAccountIfNeeded(
  options: OIDCStoredAccountAutoRefreshOptions
): Promise<OIDCStoredAccountAutoRefreshResult> {
  if (!options.adapter.getAccount || (!options.adapter.updateAccountIfMatch && !options.adapter.updateAccount)) {
    throw createAuthError(
      "PUREQ_OIDC_ADAPTER_UNSUPPORTED",
      "pureq: adapter must implement getAccount plus updateAccountIfMatch (preferred) or updateAccount for stored OIDC auto refresh"
    );
  }

  const account = await options.adapter.getAccount(options.provider, options.providerAccountId);
  if (!account) {
    return { account: null, refreshed: false };
  }

  const result = await refreshOIDCAccountIfNeeded(account, {
    flow: options.flow,
    ...(options.refreshThresholdSeconds !== undefined ? { refreshThresholdSeconds: options.refreshThresholdSeconds } : {}),
    ...(options.now ? { now: options.now } : {}),
    ...(options.onRefreshed ? { onRefreshed: options.onRefreshed } : {}),
    ...(options.onError ? { onError: options.onError } : {}),
  });

  if (result.refreshed) {
    if (options.adapter.updateAccountIfMatch) {
      const persisted = await options.adapter.updateAccountIfMatch({
        expected: account,
        next: result.account,
      });
      if (!persisted) {
        return {
          account: await options.adapter.getAccount(options.provider, options.providerAccountId),
          refreshed: false,
          conflict: true,
          ...(result.tokenResponse ? { tokenResponse: result.tokenResponse } : {}),
        };
      }
      return {
        account: persisted,
    ...
    // Legacy fallback for adapters without atomic compare-and-swap support.
    await options.adapter.updateAccount!(result.account);
  }
