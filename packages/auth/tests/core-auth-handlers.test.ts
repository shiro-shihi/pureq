...
import { credentialsProvider, emailProvider, passkeyProvider } from "../src/providers";
...
describe("core auth handlers", () => {
  ...
  it("supports passkey registration/authentication with replay and counter protections", async () => {
    const adapter = createInMemoryAdapter();
    const verifyRegistration = vi.fn(async ({ expectedChallenge }) => ({
      verified: true,
      credential: {
    ...
    expect(registerOptionsBody.challengeId).toBeTruthy();
    expect(registerOptionsBody.options.challenge).toBeTruthy();
    ...
    const authOptions = await auth.handlers.handleSignIn({
      method: "POST",
      headers: {},
      body: {
        provider: "passkey",
    ...
    const authOptionsBody = await authOptions.json();

    const authVerify = await auth.handlers.handleSignIn({
      method: "POST",
      headers: {},
      body: {
        provider: "passkey",
        passkey: {
          ...
          challengeId: authOptionsBody.challengeId,
          userId: registerOptionsBody.user.id,
          response: {
            id: "cred-passkey-1",
          },
    ...
    const replay = await auth.handlers.handleSignIn({
      method: "POST",
      headers: {},
      body: {
        provider: "passkey",
    ...
    authCounter = 4;
    const secondAuthOptions = await auth.handlers.handleSignIn({
      method: "POST",
      headers: {},
      body: {
        provider: "passkey",
    ...
    const secondAuthOptionsBody = await secondAuthOptions.json();
    const rollback = await auth.handlers.handleSignIn({
      method: "POST",
      headers: {},
      body: {
        provider: "passkey",
        passkey: {
          action: "authenticate-verify",
          challengeId: secondAuthOptionsBody.challengeId,
          userId: registerOptionsBody.user.id,
          response: {
            id: "cred-passkey-1",
          },
