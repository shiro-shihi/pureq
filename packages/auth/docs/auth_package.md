...
- passkey / WebAuthn: provider flow, one-time challenge lifecycle, sign-counter safety checks
...
## Passkey (WebAuthn) Quick Usage

```ts
import { createAuth, passkeyProvider, createInMemoryAdapter } from "@pureq/auth";
  ...
  providers: [
    passkeyProvider({
      ...
      verifyRegistration: async (params) => {
        // integrate your WebAuthn server library here
        return { verified: true, credential: params.response as any };
      },
...
Passkey routes are handled through `handleSignIn` with actions:
...
## OIDC Provider Token Lifecycle

When storing OAuth/OIDC accounts (for example, Google), use auto-refresh helpers before provider API calls:
